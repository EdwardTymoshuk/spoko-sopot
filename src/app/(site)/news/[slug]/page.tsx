...
const NewsDetailsPage = async ({ params }: NewsDetailsPageProps) => {
  ...
  return (
    <MainContainer className="pt-14 pb-24">
      <MaxWidthWrapper className="max-w-[1520px] pt-10 md:pt-36 lg:pt-40">
        <Link
          href="/news"
        ...
        <header className="mt-10 max-w-5xl border-b border-zinc-200 pb-10">
          ...
          <h1 className="mt-5 font-serif text-4xl leading-tight text-secondary md:text-6xl">
            {item.title}
          </h1>
      ...
      <MaxWidthWrapper className="max-w-[1520px] grid gap-10 py-10 md:py-14 lg:grid-cols-[minmax(0,1fr)_320px]">
        <article className="max-w-5xl">
          <div
            className="text-base leading-relaxed text-zinc-600 [&_a]:font-semibold [&_a]:text-secondary [&_a]:transition [&_a:hover]:text-primary [&_h2]:mb-4 [&_h2]:font-serif [&_h2]:text-3xl [&_h2]:text-zinc-950 [&_h3]:font-serif [&_h3]:text-2xl [&_h3]:text-zinc-950 [&_li]:mb-2 [&_p]:mb-6 [&_section]:space-y-8 [&_strong]:text-zinc-950"
        ...
        <aside className="h-fit rounded-lg border border-zinc-200 bg-white/55 p-6">
          ...
          <h2 className="mt-3 font-serif text-2xl text-zinc-950">
      ...
      {galleryImages.length > 1 ? (
        <MaxWidthWrapper className="max-w-[1520px] pb-20">
          <div className="mb-8 flex items-end justify-between gap-6">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-primary">
                Zdjęcia
              </p>
              <h2 className="mt-3 font-serif text-4xl text-secondary">
                Galeria wpisu
              </h2>
            </div>
          </div>
